...
(ns ^{:author "Dragan Djuric"}
  ...
  (:require [uncomplicate.commons
             [core :refer [Releaseable release info Wrapper wrap Wrappable extract
...
(extend-type nvrtcProgram
  Releaseable
  (release [this]
    (dragan-says-ex "It is not allowed to use and release raw nvrtcProgram. Use a safe wrapper."
                    {:this this}))
  Wrappable
  (wrap [prog]
...
(extend-type CUdeviceptr
  ...
  (with-offset [cu byte-offset]
    (.withByteOffset cu ^long byte-offset)))
...
;;TODO rewrite generically as a macro
  ...
  (let [res (JITOptions.)]
    (doseq [[option value] options]
      (put-jit-option value (or (jit-options option)
                                (dragan-says-ex "Unknown jit option."
...
(defn link-add-data* [link-state type data name options]
  (let [type (or (jit-input-types type)
                 (dragan-says-ex "Invalid jit input type." {:type type :available jit-input-types}))]
